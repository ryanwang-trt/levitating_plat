...
int imu_read(const struct device *dev, struct imu_sample *out)
{
	...
	out->accel_g[0] = ax/ACCEL_LSB_PER_G - accel_offset[0];
	out->accel_g[1] = ay/ACCEL_LSB_PER_G - accel_offset[1];
	out->accel_g[2] = az/ACCEL_LSB_PER_G - accel_offset[2];

	out->gyro_dps[0] = gyroX/GYRO_LSB_PER_DPS - gyro_bias[0];
	out->gyro_dps[1] = gyroY/GYRO_LSB_PER_DPS - gyro_bias[1];
	out->gyro_dps[2] = gyroZ/GYRO_LSB_PER_DPS - gyro_bias[2];
...
void imu_calibrate(const struct device *dev)
{
	...
	for (int n = 0; n < samples; n++) {
		struct imu_sample s;
		if (imu_read(dev, &s) == 0) {   // biases are still 0 -> raw
			for (int i = 0; i < 3; i++) {
				accel_sum[i] += s.accel_g[i];
				gyro_sum[i] += s.gyro_dps[i];
			}
			valid++;
		}
		k_sleep(K_MSEC(5));
